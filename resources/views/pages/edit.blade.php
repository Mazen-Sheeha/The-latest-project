                    ...
                    <div class="border-t pt-4 mt-4">
                        ...
                        @if (isset($pixels) && $pixels->count() > 0)
                            @php
                                ...
                                $selectedPixelIds = old('pixels', $page->pixels->pluck('id')->toArray());
                            @endphp
                            ...
                            <select name="pixels[]" multiple id="pixels-select">
                                    ...
                                    <optgroup label="{{ $pixelTypes[$type] ?? $type }}">
                                        @foreach ($typePixels as $pixel)
                                            <option value="{{ $pixel->id }}"
                                                {{ in_array($pixel->id, $selectedPixelIds) ? 'selected' : '' }}>
                        ...
                        @else
                            <div class="text-center p-12 text-gray-400 border border-dashed rounded-xl">
                                <p class="text-sm">لا توجد بكسلات مضافة.</p>
                    ...
                    <div class="card-body p-6 space-y-4">
                        ...
                        <div id="sale-fields" class="{{ $page->sale_price ? '' : 'hidden' }} grid md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" id="sale_price" step="0.01" name="sale_price"
                                    class="input w-full" value="{{ old('sale_price', $page->sale_price) }}">
                            </div>
                                        ...
                                        <div class="border rounded p-4 space-y-3 bg-gray-50">
                                            <div class="flex justify-between items-center mb-2">
                                                <div class="flex items-center gap-4">
                                                    ...
                                                    <label
                                                        class="flex items-center gap-2 cursor-pointer bg-white px-3 py-1 rounded-full border text-xs font-bold shadow-sm">
                                                        <input type="checkbox"
                                                            name="offers[{{ $index }}][selected]" value="1"
                                                            class="offer-selected-checkbox"
                                                            {{ isset($offer['selected']) && $offer['selected'] ? 'checked' : '' }}>
                                                        تفعيل كخيار افتراضي
                                                    </label>
                                                </div>
                                                <button type="button"
                                            ...
                                            <div>
                                                <label class="form-label text-xs font-bold">الجملة الكاملة للعرض
                                                    (اختياري)
                                                </label>
                                                <input type="text" name="offers[{{ $index }}][sentence]"
                                                    class="input w-full" value="{{ $offer['sentence'] ?? '' }}"
                                                    placeholder="مثلاً: اشتري 2 قطعة بسعر 180 د.إ فقط">
                                            </div>
                                                ...
                                                <div class="flex gap-3">
                                                    ...
                                                    <div class="relative">
                                                        ...
                                                        <img class="offer-image-preview w-20 h-20 rounded border object-cover"
                                                            src="{{ isset($offer['image']) && $offer['image'] ? asset('public/' . ltrim($offer['image'], '/')) : 'https://via.placeholder.com/80' }}"
                                                            alt="Offer Image">
                                                    </div>
                                                </div>
                                            </div>
                ...
                <div class="card bg-white shadow rounded-lg">
                    ...
                    <div class="card-body p-6 space-y-3">
                        @php
                            ...
                            $savedFeatures = $page->features ?? [];
                            $featuresMap = [];

                            foreach ($savedFeatures as $k => $v) {
                                if (is_array($v)) {
                                    // was saved as [{value: 'cod', label: '...', image: '...'}]
                                    $key = (string) ($v['value'] ?? $k);
                                    $label = (string) ($v['label'] ?? ($featureDefaults[$key] ?? $key));
                                    $featuresMap[$key] = $label;
                                } elseif (is_int($k) || is_numeric($k)) {
                                    // old flat format: ['cod', 'free_shipping']
                                    $featuresMap[(string) $v] = $featureDefaults[(string) $v] ?? (string) $v;
                                } else {
                                    // associative format: ['cod' => 'label']
                                    $featuresMap[(string) $k] = (string) $v;
                                }
                            }
                        @endphp
                        @foreach ($featureDefaults as $value => $defaultLabel)
                            <div class="flex items-center gap-3 border rounded-lg p-3">
                                <input type="checkbox" name="features_active[]" value="{{ $value }}"
                                    ...
                                    {{ array_key_exists($value, $featuresMap) ? 'checked' : '' }}>

                                <input type="text" name="features_labels[{{ $value }}]" class="input flex-1"
                                    value="{{ $featuresMap[$value] ?? $defaultLabel }}"
                            ...
                            @foreach ($page->images ?? [] as $index => $img)
                                <div class="flex items-center justify-between border rounded p-4 bg-gray-50 cursor-move"
                                    ...
                                    bg-blue-600 text-white text-sm font-bold rounded-full">
                                        ...
                                        <img src="{{ asset('public/' . ltrim($img, '/')) }}"
                            ...
                            <div class="border rounded p-4 space-y-3 bg-gray-50 review-item flex flex-col gap-4">
                                ...
                                <div class="flex items-center gap-3 relative">
                                    @if ($review->reviewer_image)
                                        <img src="{{ asset('public/' . ltrim($review->reviewer_image, '/')) }}"
                                ...
                                @foreach ($page->upsellProducts as $product)
                                    <div class="border rounded-lg p-4 space-y-4 bg-gray-50 flex flex-col gap-4">
                                        ...
                                        <div>
                                            ...
                                            <select name="upsell_products[{{ $loop->index }}][product_id]"
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="upsell_products[{{ $loop->index }}][name]"
                                        ...
                                        <div>
                                            ...
                                            <div class="flex gap-3 relative">
                                                <input type="file" name="upsell_products[{{ $loop->index }}][image]"
                                                    class="input w-full product-image-file" accept="image/*">
                                                <img class="product-image-preview w-20 h-20 rounded border object-cover"
                                                    src="{{ $product->pivot->image ? asset('public/' . ltrim($product->pivot->image, '/')) : asset('images/productDefault.webp') }}"
                                                    alt="Product Image">
                                                <button type="button"
                                        ...
                                        <div>
                                            ...
                                            <input type="number" step="0.01"
                                                name="upsell_products[{{ $loop->index }}][price]"
...
@section('script')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const pixelSelectEl = document.getElementById('pixels-select');
            if (!pixelSelectEl) return;
    ...
    <script>
        ...
        <div>
    <label class="form-label text-xs font-bold">الجملة الكاملة للعرض (اختياري)</label>
    <input type="text" name="offers[${offerIndex}][sentence]"
           class="input w-full"
           placeholder="مثلاً: اشتري 2 قطعة بسعر 180 د.إ فقط">
</div>
    ...
    `;
        ...
        // Handle "Select Only One" logic (Delegated)
        document.addEventListener('change', function(e) {
            if (e.target && e.target.classList.contains('offer-selected-checkbox')) {
                if (e.target.checked) {
            ...
            <select name="upsell_products[${upsellIndex}][product_id]" class="input w-full product-select" required>
                ...
                ${allProducts.map(p => `
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        <option value="${p.id}"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                data-name="${p.name}"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                data-price="${p.price}"
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                data-image="${p.image ? '{{ asset('public') }}' + p.image : '{{ asset('images/productDefault.webp') }}'}">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            ${p.name}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        </option>
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    `).join('')}
            </select>
