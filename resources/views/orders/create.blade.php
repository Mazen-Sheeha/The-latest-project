...
@section('content')
    <div class="card pb-2 5">
        <form class="card-body grid gap-5" action="{{ route('orders.store') }}" method="post" id="form">
            @csrf
            ...
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                ...
                <input type="text" class="input" name="phone" placeholder="رقم الهاتف" value="{{ old('phone') }}">
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">المدينة</label>
                <input type="text" class="input" name="city" value="{{ old('city') }}" placeholder="المدينة">
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">العنوان</label>
                <textarea class="textarea" name="address" rows="5" placeholder="العنوان">{{ old('address') }}</textarea>
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">رقم التتبع (اختياري)</label>
                <input type="text" class="input" name="tracking_number" value="{{ old('tracking_number') }}"
                    placeholder="رقم التتبع">
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">الرابط</label>
                <input class="input" name="url" placeholder="الرابط" value="{{ old('url') }}" />
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">سعر شركة الشحن (اختياري)</label>
                <input type="number" class="input" min="0" name="shipping_price" placeholder="السعر"
                    value="{{ old('shipping_price') }}" id="shipping_price" />
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">ملاحظات</label>
                <textarea class="textarea" name="notes" placeholder="دون ملاحظات على الطلب" id="notes" rows="6">{{ old('notes') }}</textarea>
            </div>
                ...
                <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                    ...
                    <select name="campaign_id" id="select-beast" autocomplete="off" style="width: 100%">
                        ...
                        @foreach ($campaigns as $campaign)
                            <option value="{{ $campaign->id }}">اسم الحملة : {{ $campaign->campaign }} | المصدر :
            ...
            @if ($products->count() > 0)
                <div class="products">
                    @php
                        $oldProducts = old('product_ids', [null]);
                    @endphp
                        ...
                        <div class="product">
                            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                                ...
                                <div class="w-full mb-3">
                                    <select name="product_ids[]" class="w-full data-attr mb-3 pros">
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}" @selected($productId == $product->id)
                                                data-src='{{ $product->image() }}' code="{{ $product->code }}"
                                    ...
                                    <div class="flex gap-2 pro-data">
                                        <input type="number" class="input mb-3 quantity"
                                            ...
                                            value="{{ old('quantities.' . ($productId ?? '')) }}" placeholder="الكمية">
                                        <input type="text" class="input w-56 price"
                                            name="prices[{{ $productId ?? '' }}]"
                                            value="{{ old('prices.' . ($productId ?? '')) }}" placeholder="السعر">
                                        <input type="hidden" class="input w-56 code"
                                            name="codes[{{ $productId ?? '' }}]"
                                            value="{{ old('codes.' . ($productId ?? '')) }}" placeholder="السعر">
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.querySelector("#form").addEventListener("submit", function(e) {
            document.querySelectorAll(".product").forEach(function(productDiv) {
                ...
                const code = select.options[select.selectedIndex].getAttribute('code');
                const productId = select.value;
                const priceInput = productDiv.querySelector(".price");
                const quantityInput = productDiv.querySelector(".quantity");
                const codeInput = productDiv.querySelector(".code");

                if (priceInput && quantityInput && productId) {
                    priceInput.name = `prices[${productId}]`;
                    quantityInput.name = `quantities[${productId}]`;
                    codeInput.name = `codes[${productId}]`;
                    codeInput.value = code;
                }

                const shippingPirceInp = document.getElementById("shipping_price");
                const firstSelect = document.querySelector('select.pros');
                shippingPirceInp.value = firstSelect.options[firstSelect.selectedIndex].getAttribute(
                    'shippingPrice');
            });
        })
