...
@section('url_pages')
    ...
    <i class="ki-filled ki-left text-gray-500 text-3xs"></i>
    <span class="text-gray-700">تقرير مجموعات الحملات الإعلانية</span>
@endsection

@section('content')
    <form action="{{ route('adsets.statistics') }}" method="GET" class="flex gap-3 flex-col flex-wrap" id="searchForm">
        <div class="flex gap-3 items-center">
            ...
            <button type="submit" id="searchBtn" class="btn btn-primary flex justify-center max-w-56">
                <span id="searchText">بحث</span>
                <div id="searchSpinner" class="loading-spinner"></div>
            </button>

            @if (request()->hasAny(['from', 'to', 'source', 'active']))
                <a href="{{ route('adsets.statistics') }}" class="btn btn-secondary flex justify-center max-w-56">تراجع</a>
            @endif
        </div>
    </form>
    ...
    <div class="card min-w-full mt-4">
        <div class="card-header">
            <h3 class="card-title">تقرير مجموعات الحملات الإعلانية</h3>
        </div>
            ...
            <div class="scrollable-auto">
                    ...
                    <table class="table align-middle text-2sm text-gray-600" id="adsets-table">
                        <thead>
                            <tr class="bg-gray-100">
                                ...
                                <th class="text-start font-medium min-w-15">المجموعة</th>
                        ...
                        <tbody>
                            @foreach ($adsets as $adset)
                                @php
                                    $stats = $adset->calculateStatistics();
                                @endphp
                                <tr>
                                    <td>
                                        <form action="{{ route('adsets.changeActive', $adset->id) }}" method="POST"
                                            class="active-form" style="margin: 0">
                                            @csrf
                                            <button
                                                class="on-off @if ($adset->active) on @else off @endif"></button>
                                        </form>
                                    </td>
                                    <td class="adset-name text-nowrap">
                                        <a href="{{ route(
                                            'campaigns.statistics',
                                            array_merge(['adset_id' => $adset->id], request()->only(['from', 'to', 'source', 'active'])),
                                        ) }}"
                                            style="color:rgb(114, 114, 255); text-decoration: underline">
                                            {{ $adset->name }}
                                        </a>
                                    </td>
                                    <td class="text-nowrap">AED {{ $adset->budgets_sum_budget ?? 0 }}</td>
                ...
                @else
                    <div class="p-4 text-center bg-white">
                        @if (request()->hasAny(['from', 'to', 'source', 'active']))
                            <h5 class="text-gray-600">لا توجد مجموعات حملات إعلانية تطابق معايير البحث</h5>
                            <a href="{{ route('adsets.statistics') }}" class="text-blue-500 hover:underline">إعادة تعيين
                                الفلتر</a>
                        @else
                            <h5 class="text-gray-600">لا توجد مجموعات حملات إعلانية</h5>
                        @endif
                    </div>
                @endif
            </div>
