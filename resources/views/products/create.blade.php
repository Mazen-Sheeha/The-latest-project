...
@section('content')
    <div class="card pb-2 5">
        <form class="card-body grid gap-5" id="create-product" action="{{ route('products.store') }}" method="post"
            enctype="multipart/form-data">
            @csrf
            <div class="flex items-baseline justify-center flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label for="image" class="cursor-pointer">
                    <img loading="lazy" src="{{ asset('images/upload_area.png') }}" class="image-label " alt="Upload here"
            ...
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                ...
                <input type="text" class="input" name="code" placeholder="الكود" value="{{ old('code') }}">
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">سعر المنتوج</label>
                <input type="text" class="input" id="price" name="price" value="{{ old('price') }}" placeholder="السعر">
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">المخزون</label>
                <input type="number" class="input" name="stock" value="{{ old('stock') }}"
                    placeholder="كم عدد القطع المتوفرة ؟">
            </div>
            <div class="flex items-baseline flex-wrap lg:flex-nowrap gap-2.5 mb-2.5">
                <label class="form-label max-w-56">شركة الشحن</label>
                <select name="shipping_company_id" class="select">
                    @foreach ($companies as $company)
                        <option value="{{ $company->id }}" @selected(old('shipping_company_id') === $company->id)>{{ $company->name }}</option>
                    @endforeach
                </select>
            </div>
...
@section('script')
    <script>
        ...
        document.querySelector("#create-product").addEventListener("submit", () => {
            Swal.fire({
                title: 'جاري التحميل ...',
                text: 'برجاء الانتظار',
