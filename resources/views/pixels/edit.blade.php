...
@section('url_pages')
    ...
    <i class="ki-filled ki-left"></i>
    <span>تعديل البكسل</span>
@endsection

@section('content')
    <div class="space-y-6">

        <form action="{{ route('pixels.update', $pixel) }}" method="POST" id="pixel-form">
            @csrf
            @method('PUT')

            <div class="card bg-white shadow rounded-lg">
                <div class="card-header">
                    <h3 class="card-title">تعديل البكسل</h3>
                </div>
                ...
                <div class="card-body p-6 space-y-4">
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="name" class="input w-full" required value="{{ old('name', $pixel->name) }}">
                            @error('name')
                        ...
                        <div>
                            ...
                            <select name="type" class="input w-full" required>
                                ...
                                @foreach ($types as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('type', $pixel->type) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input name="pixel_id" class="input w-full" required
                                value="{{ old('pixel_id', $pixel->pixel_id) }}" placeholder="مثال: 123456789012345">
                            @error('pixel_id')
                                <div class="text-danger text-sm mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-end">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="is_active" value="1"
                                    {{ old('is_active', $pixel->is_active) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea name="code" class="input w-full font-mono text-xs" rows="6"
                            placeholder="أدخل كود البكسل الكامل إذا كان متاحاً">{{ old('code', $pixel->code) }}</textarea>
